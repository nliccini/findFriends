...
%% Create Output Image
    ...
    % clrs is pure white
    clrs = [255 255 255];
    % The tagging is a bit different with 1 dimensional images
    for ndx=1:numfaces
        friend = imgs{ndx};
        ...
        [trows, ~, ~] = size(nameimg);
        % Since this is 1-dimensional, just edit the one layer
        if l == 1
            layer1 = friend((end-trows)+1:end,:);
            layer2 = nameimg(:,:,1);
            mask = layer2 == 255;
            layer1(mask) = layer2(mask);
            friend((end-trows)+1:end,:) = layer1;
            % Store each image as a new element in a cell array
            friends = [friends, {friend}];
        else
            % Use iteration to insert the text image to the bottom of the face
            for clr=1:3
                % Find the locations where the text will go on the face
                layer1 = friend((end-trows)+1:end,:,clr);
                % Grab all of the image of that layer
                layer2 = nameimg(:,:,clr);
                % Mask where that layer is full brightness (where text is)
                mask = layer2 == clrs(clr);
                % Make those locations on the face the colors of the text
                layer1(mask) = layer2(mask);
...
%% Determine if User Wants a Specific Face
    ...
    all_Names = {faces.Name};
    disp(all_Names);
    % Continue to ask for more pics if they specify a name
    seeMore('Want to see anyone in particular? [name/no]\n', faces);
...
function [data, imported] = makeData(question)
    ...
    switch reply
        ...
        otherwise
            ...
            [data, imported] = makeData(question);
    end
end
...
% Helper function to show the first image of the specified name
function [] = seeMore(question, faces)
    reply = input(question, 's');
